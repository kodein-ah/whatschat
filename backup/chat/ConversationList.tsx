...
interface Props {
  ...
  meId: string;
  conversations?: Conversation[]; // ✅ NEW: Optional prop untuk manual conversations
}
...
export const ConversationList = ({ activeId, onSelect, meId, conversations: propConversations }: Props) => {
  // ✅ FIX: Query conversations hanya jika tidak ada prop
  const { 
    ...
    enabled: !propConversations, // ✅ Only fetch if prop not provided
  });

  // ✅ FIX: Use prop conversations jika ada, else use fetched
  ...
  return (
    <ul className="scrollbar-thin flex-1 overflow-y-auto bg-background">
      {conversations.map((c) => {
        ...
        if (!c || !c.id) {
          console.warn("⚠️ [ConversationList] Invalid conversation object:", c);
          return null;
        }

        // ✅ FIX: Safe ID conversions
